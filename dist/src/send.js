...
function shouldSuppressOutbound(threadId, isGroup) {
    if (isGroup)
        return false;
    try {
...
function applyInlineStyles(text) {
    ...
    while (i < work.length) {
        if (work.charCodeAt(i) === 0x0001) {
            ...
            out += entry.content;
            styles.push({ start, len: entry.content.length, st: entry.style });
            i = end + 1;
        }
        else {
            out += work[i++];
...
export function markdownToZaloStyles(input) {
    let src = input;
    ...
    const styles = [];
    let cursor = 0;
    const pushBlockStyle = (start, len, st) => {
        if (len > 0)
...
export async function sendMessageZaloPersonal(threadId, text, options = {}) {
    if (!threadId?.trim()) {
...
async function sendMediaZaloPersonal(threadId, mediaUrl, options = {}) {
    if (!threadId?.trim()) {
        return { ok: false, error: "No threadId provided" };
    }
    if (!mediaUrl?.trim()) {
...
export async function sendLinkZaloPersonal(threadId, url, options = {}) {
    if (!threadId?.trim()) {
        return { ok: false, error: "No threadId provided" };
    }
    if (!url?.trim()) {
