import { applyAccountNameToChannelSection, buildChannelConfigSchema, DEFAULT_ACCOUNT_ID, deleteAccountFromConfigSection, formatPairingApproveHint, migrateBaseNameToDefaultAccount, normalizeAccountId, setAccountEnabledInConfigSection, } from "openclaw/plugin-sdk/channel-plugin-common";
import { listZaloPersonalAccountIds, resolveDefaultZaloPersonalAccountId, resolveZaloPersonalAccountSync, getZaloPersonalUserInfo, checkZaloPersonalAuthenticated, } from "./accounts.js";
...
function resolveZaloPersonalGroupRequireMention(params) {
...
function resolveZaloPersonalGroupToolPolicy(params) {
    const account = resolveZaloPersonalAccountSync({
        cfg: params.cfg,
        accountId: params.accountId ?? undefined,
    });
...
export const zaloPersonalDock = {
    ...
    config: {
        resolveAllowFrom: ({ cfg, accountId }) => (resolveZaloPersonalAccountSync({ cfg: cfg, accountId }).config.allowFrom ?? []).map((entry) => String(entry)),
...
export const zaloPersonalPlugin = {
    id: "zalo-personal",
    meta,
    ...
    directory: {
        ...
        listGroups: async ({ cfg, accountId, query, limit }) => {
            ...
            if (groupIds.length > 0) {
                try {
                    ...
                    rows = Object.entries(gridInfoMap).map(([id, info]) => mapGroup({
                        id,
                        name: info.name ?? null,
                        raw: info,
                    }));
        ...
        listGroupMembers: async ({ cfg, accountId, groupId, limit }) => {
            ...
            try {
                ...
                const rows = Object.entries(profiles).map(([id, profile]) => mapUser({
                    id,
            ...
            for (const input of inputs) {
                ...
                try {
                    ...
                    if (kind === "user") {
                        ...
                        const matches = groups.filter((g) => (g.name ?? "").toLowerCase().includes(trimmed.toLowerCase()));
