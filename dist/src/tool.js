...
import { ThreadType, Reactions, MuteAction, UpdateSettingsType } from "zca-js";
import { getApi } from "./zalo-client.js";
import { readOpenClawConfig, writeOpenClawConfig, addToDenyFrom, removeFromDenyFrom, addToGroupDenyUsers, removeFromGroupDenyUsers, addToGroupAllowUsers, removeFromGroupAllowUsers, listBlockedUsers, listAllowedUsers, listAllowedUsersInGroup, setGroupRequireMention, getGroupRequireMention, } from "./config-manager.js";
 ...
 */
function extractMemberIds(info) {
    ...
    if (memVerList.length > 0) {
        return memVerList.map((entry) => entry.split("_")[0]).filter(Boolean);
    }
    return [];
}
const ACTIONS = [
...
export const ZaloPersonalToolSchema = Type.Object({
    ...
    styles: Type.Optional(Type.Array(Type.Object({
...
async function resolveUserId(nameOrId) {
    ...
    const match = friendList.find((f) => (f.displayName ?? "").toLowerCase() === query ||
...
async function resolveGroupId(nameOrId) {
    ...
    try {
        ...
        const query = nameOrId.toLowerCase();
        const match = Object.entries(gridInfoMap).find(([_id, info]) => (info.name ?? "").toLowerCase() === query);
        if (match) {
...
export async function executeZaloPersonalTool(_toolCallId, params, _signal, _onUpdate) {
    try {
        switch (params.action) {
            ...
            case "image": {
                ...
                const api = await getApi();
                const type = params.isGroup ? ThreadType.Group : ThreadType.User;
                const result = await api.sendLink({ url: params.url, title: params.message || params.url }, params.threadId, type);
                return json({ success: true, messageId: result?.message?.msgId });
            }
            case "link": {
                ...
                const api = await getApi();
                const type = params.isGroup ? ThreadType.Group : ThreadType.User;
                const result = await api.sendLink({ url: params.url }, params.threadId, type);
                return json({ success: true, messageId: result?.message?.msgId });
            }
            case "friends": {
                const api = await getApi();
                ...
                if (params.query?.trim()) {
                    ...
                    friendList = friendList.filter((f) => (f.displayName ?? "").toLowerCase().includes(q) ||
                        (f.zaloName ?? "").toLowerCase().includes(q) ||
                        String(f.userId).includes(q));
                }
                const mapped = friendList.map((f) => ({
                    userId: f.userId,
            ...
            case "search-groups": {
                ...
                try {
                    const infoResp = await api.getGroupInfo(groupIds);
                    const gridInfoMap = infoResp?.gridInfoMap ?? {};
                    let groups = Object.entries(gridInfoMap).map(([id, info]) => ({
                        groupId: id,
                        name: info.name,
            ...
            case "find-user": {
                ...
                if (!/^(\+84|84|0)\d{9,10}$/.test(cleanPhone)) {
                    throw new Error(`Invalid phone number format: ${params.phoneNumber}. Expected: 0987654321 or +84987654321`);
                }
                const api = await getApi();
            ...
            case "send-friend-request": {
                ...
                if (!/^\d+$/.test(params.userId)) {
                    throw new Error(`Invalid userId: ${params.userId}. Use numeric ID from find-user action.`);
                }
            ...
            case "send-to-stranger": {
                ...
                const result = await api.sendMessage({ msg: params.message }, params.userId, ThreadType.User);
                return json({
                    success: true,
                    userId: params.userId,
                    messageId: result?.message?.msgId,
            ...
            case "get-sent-requests": {
                ...
                const requests = Object.entries(response).map(([uid, info]) => ({
            ...
            case "send-voice": {
                ...
                const api = await getApi();
                const type = params.isGroup ? ThreadType.Group : ThreadType.User;
                const result = await api.sendVoice({ voiceUrl: params.voiceUrl }, params.threadId, type);
                return json({
                    success: true,
                    messageId: result?.msgId,
            ...
            case "send-sticker": {
                ...
                const api = await getApi();
                const type = params.isGroup ? ThreadType.Group : ThreadType.User;
                const result = await api.sendSticker({ id: params.stickerId, cateId: params.stickerCateId, type: 2 }, params.threadId, type);
                return json({
                    success: true,
                    messageId: result?.msgId,
            ...
            case "send-card": {
                ...
                const api = await getApi();
                const type = params.isGroup ? ThreadType.Group : ThreadType.User;
                const result = await api.sendCard({ userId: params.userId, phoneNumber: params.phoneNumber }, params.threadId, type);
                return json({
                    success: true,
                    messageId: result?.msgId,
            ...
            case "undo-message": {
                ...
                const api = await getApi();
                const type = params.isGroup ? ThreadType.Group : ThreadType.User;
                const result = await api.undo({ msgId: params.msgId, cliMsgId: params.cliMsgId }, params.threadId, type);
                return json({
                    success: true,
                    status: result?.status,
            ...
            case "add-reaction": {
                ...
                const reactionIcon = iconMap[iconStr] ?? iconStr;
                const api = await getApi();
                const type = params.isGroup ? ThreadType.Group : ThreadType.User;
            ...
            case "create-poll": {
                ...
                const groupId = await resolveGroupId(params.groupId);
                const api = await getApi();
                const result = await api.createPoll({ question: params.title, options: params.options }, groupId);
                return json({
                    success: true,
                    poll: result,
            ...
            case "update-group-settings": {
                ...
                const groupId = await resolveGroupId(params.groupId);
                const api = await getApi();
                await api.updateGroupSettings(params.groupSettings, groupId);
                return json({
                    success: true,
                    groupId,
            ...
            case "review-pending-members": {
                ...
                const groupId = await resolveGroupId(params.groupId);
                const api = await getApi();
                const result = await api.reviewPendingMemberRequest({ members: params.memberIds, isApprove: params.isApprove }, groupId);
                return json({
                    success: true,
                    groupId,
            ...
            case "mute-conversation": {
                ...
                await api.setMute({ duration: dur, action: MuteAction.MUTE }, params.threadId, type);
                return json({
                    success: true,
                    threadId: params.threadId,
                    duration: dur,
            ...
            case "unmute-conversation": {
                ...
                const api = await getApi();
                const type = params.isGroup ? ThreadType.Group : ThreadType.User;
                await api.setMute({ action: MuteAction.UNMUTE }, params.threadId, type);
                return json({
                    success: true,
                    threadId: params.threadId,
            ...
            case "delete-chat": {
                ...
                const type = params.isGroup ? ThreadType.Group : ThreadType.User;
                const ownId = api.getOwnId();
                const result = await api.deleteChat({ ownerId: ownId, cliMsgId: params.cliMsgId, globalMsgId: params.msgId }, params.threadId, type);
                return json({
                    success: true,
                    status: result?.status,
            ...
            case "update-setting": {
                ...
                const validKeys = Object.values(UpdateSettingsType);
                if (!validKeys.includes(params.settingKey)) {
                    throw new Error(`Invalid settingKey: ${params.settingKey}. Valid keys: ${validKeys.join(", ")}`);
                }
                const api = await getApi();
                await api.updateSettings(params.settingKey, params.settingValue);
                return json({
                    success: true,
            ...
            case "search-stickers": {
                ...
                return json({
                    stickers: (Array.isArray(details) ? details : []).map((s) => ({
                        id: s.id,
                        cateId: s.cateId,
                        type: s.type,
                        text: s.text,
            ...
            case "send-report": {
                ...
                const reportOptions = reasonNum === 0
                    ? { reason: 0, content: params.message || "Reported" }
                    : { reason: reasonNum };
            ...
            case "delete-group-invite": {
                ...
                const api = await getApi();
                const result = await api.deleteGroupInviteBox(params.groupId, params.blockFutureInvite ?? false);
                return json({
                    success: true,
            ...
            case "get-group-members-info": {
                ...
                return json({
                    profiles: Object.entries(profiles).map(([id, info]) => ({
                        userId: id,
            ...
            case "set-auto-delete-chat": {
                ...
                const api = await getApi();
                const type = params.isGroup ? ThreadType.Group : ThreadType.User;
                await api.updateAutoDeleteChat(params.ttl, params.threadId, type);
                return json({
                    success: true,
                    threadId: params.threadId,
                    ttl: params.ttl,
            ...
            case "create-note": {
                ...
                const groupId = await resolveGroupId(params.groupId);
                const api = await getApi();
                const result = await api.createNote({ title: params.title, pinAct: params.pinAct }, groupId);
                return json({
                    success: true,
                    note: result,
            ...
            case "edit-note": {
                ...
                const groupId = await resolveGroupId(params.groupId);
                const api = await getApi();
                const result = await api.editNote({ title: params.title, topicId: params.topicId, pinAct: params.pinAct }, groupId);
                return json({
                    success: true,
                    note: result,
            ...
            case "update-quick-message": {
                ...
                const api = await getApi();
                const result = await api.updateQuickMessage({ keyword: params.keyword, title: params.title }, params.itemId);
                return json({
                    success: true,
                    item: result?.item,
            ...
            default: {
                params.action;
                throw new Error(`Unknown action: ${String(params.action)}. Valid actions: ${ACTIONS.join(", ")}`);
            }
        }
    }
    catch (err) {
